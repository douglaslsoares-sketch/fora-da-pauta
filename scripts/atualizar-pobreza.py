...
OUT = ROOT / "data" / "economia" / "gerado" / "pobreza.json"
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    for ano in range(2019, 2025):
        ...
        valor = float(
            aba.cell_value(linha_brasil, COL_POBREZA)
        )
...
documento = {
    "id": "pobreza",
    "titulo": "População abaixo da linha de pobreza",
    "unidade": "%",
    "metodologia": (
        "Proporção de pessoas com rendimento domiciliar "
        "per capita inferior à linha de US$ 6,85 PPC 2017 "
        "por dia, conforme critério utilizado pelo IBGE "
        "e Banco Mundial."
    ),
    "fonte": {
        "instituicao": "IBGE",
        ...
        "tabela": "2.18",
        "linha": "Menos de US$ 6,85 PPC 2017",
        "url": ZIP_URL
    },
    "anos": anos,
...
if len(teste["anos"]) != 6:
    raise RuntimeError(
        "Quantidade inesperada de anos em pobreza."
    )

tmp.replace(OUT)

print()
print("Pobreza atualizada com sucesso.")
print()
